...
public class Week06TicTacToeLab {
	...
	public static void main(String[] args) {
			...
			char[][] gameBoard = {{'1','2','3'}, {'4','5','6'},{'7','8','9'}};  // Initialize board array char
			int playerNum = 1;
			...
			for (int numMoves = 0; numMoves < 9; numMoves ++) { 
				...
				System.out.println("Player " + playerNum  +": Choose a square by number: ");
				int move = sc.nextInt();
				makeMove(gameBoard, move, playerNum);
				displayBoard(gameBoard);
				playerNum = numMoves  % 2;
				} else {//end if
				break;
	...
	public static char[][] makeMove(char[][] gameBoard, int move, int playerNum) {
		
		switch (move) {
			case (1): {if (playerNum == 1) { 
				gameBoard[0][0] = 'X';
			} else { 
				gameBoard[0][0] = 'O';
				} //end else	
			} break;
			case (2): {if (playerNum == 1) { 
				gameBoard[0][1] = 'X';
			} else { 
				gameBoard[0][1] = 'O';
				} //end else	
			} break;
			case (3): {if (playerNum == 1) { 
				gameBoard[0][2] = 'X';
			} else { 
				gameBoard[0][2] = 'O';
				} //end else	
			} break;
			case (4): {if (playerNum == 1) { 
				gameBoard[1][0] = 'X';
			} else { 
				gameBoard[1][0] = 'O';
				} //end else	
			} break;
			case (5): {if (playerNum == 1) { 
				gameBoard[1][1] = 'X';
			} else { 
				gameBoard[1][1] = 'O';
				} //end else	
			} break;
			case (6): {if (playerNum == 1) { 
				gameBoard[1][2] = 'X';
			} else { 
				gameBoard[1][2] = 'O';
				} //end else	
			} break;
			case (7): {if (playerNum == 1) { 
				gameBoard[2][0] = 'X';
			} else { 
				gameBoard[2][0] = 'O';
				} //end else	
			} break;
			case (8): {if (playerNum == 1) { 
				gameBoard[2][1] = 'X';
			} else { 
				gameBoard[2][1] = 'O';
				} //end else	
			} break;
			case (9): {if (playerNum == 1) { 
				gameBoard[2][2] = 'X';
			} else { 
				gameBoard[2][2] = 'O';
				} //end else	
	...
	public static void displayMap() { //Set initial image of the playing grid
		System.out.println("Welcome to TicTacToe! This is your playing grid:");
		System.out.println("\t\t+---+---+---+");
	 ...
	 public static boolean checkWinner(char[][] gameBoard) {
		 ...
		 if (((gameBoard[0][0] == gameBoard[1][1]) && (gameBoard[1][1] == gameBoard[2][2])) || ((gameBoard[2][0] == gameBoard[1][1]) && (gameBoard[1][1] == gameBoard[0][2]))){ // check diagonal
			 return true;  
		 } else {
				 for (int i =0; i < 3; i++) {
				 	if (((gameBoard[i][0] == gameBoard[i][1]) && (gameBoard[i][0] == gameBoard[i][2]))  || ((gameBoard[0][i] == gameBoard[1][i]) && (gameBoard[0][i] == gameBoard[2][i]))) { // check horizontal and vertical
				 		return true;
				 } // end if
